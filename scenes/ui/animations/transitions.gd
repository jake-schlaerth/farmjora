...
var _deferred_callbacks: Array[Callable]

func _ready() -> void:
	signal_bus.transition.connect(on_transition)
	texture_rect.visible = false

func on_transition(deferred_callbacks: Array[Callable] = []) -> void:
...
func set_deferred_callbacks(deferred_callbacks: Array[Callable]) -> void:
